...
const VENDORS = [
  ...
  // 常用（人工）で入ってもらう協力会社。労務費はこの2社からの請求として発生する。
  // 自社職人の出面入力は未実装のため、デモに出る労務費は必ず請求書で入るものに限る。
  { code: "V011", name: "和泉建工",             closing: 20, months: 1, payday: 25 },
  { code: "V012", name: "早瀬工業",             closing: 20, months: 1, payday: 25 },
  { code: "V013", name: "エコクリーン中国株式会社", closing: 99, months: 1, payday: 25 },
];
...
const COSTS = {
  "A-101": [
    ...
    ["2026-04-28", "0680", "labor",       "V011", "大工 常用 22人工",           1760000],
    ...
    ["2026-07-22", "0810", "expense",     "V013", "産業廃棄物 処分費",           280000],
  ],
  "A-102": [
    ...
    ["2026-07-10", "0810", "labor",       "V012", "現場常駐 世話役 常用 18人工", 1650000],
    ["2026-07-24", "0810", "expense",     "V013", "産業廃棄物 処分費・現場清掃",  640000],
  ],
  "A-103": [
    ...
    ["2026-07-18", "0680", "labor",       "V011", "大工 常用 5人工",             350000],
  ],
  "A-104": [
    ...
    ["2026-06-27", "0810", "labor",       "V012", "雑工 常用 6人工",             420000],
    ["2026-07-20", "0810", "expense",     "V013", "産業廃棄物 処分費",           180000],
  ],
};
...
async function main() {
  ...
  for (const p of PROJECTS) {
    ...
    // 原価はすべて仕入伝票から発生させる。
    // 画面に出る数字が「今のCMCで実際に入力できるもの」だけになるようにするため。
    // （原価の直接入力UIは無く、自社職人の出面入力も未実装。相手先のない原価を
    //   デモに出すと「これはどうやって入れるのか」に答えられなくなる）
    let actualTotal = 0;
